...
SMBTree.prototype.exists = function (name, cb) {
  this.spiTree.exists(consts.normalizeSMBFileName(name), cb);
};
...
SMBTree.prototype.openOrCreate = function (name, createDisposition, openTargetDirectory, cb) {
  ...
  name = consts.normalizeSMBFileName(name);
  ...
  function open(created, callback) {
    self.spiTree.open(name, function (err, file) {
      ...
      } else if (createDisposition === consts.FILE_OVERWRITE) {
...
SMBTree.prototype.open = function (name, cb) {
  var self = this;
  this.spiTree.open(consts.normalizeSMBFileName(name), function (err, file) {
    if (err) {
      cb(err);
    } else {
      var fid = ++SMBTree.fidCounter;
...
SMBTree.prototype.list = function (pattern, cb) {
  this.spiTree.list(consts.normalizeSMBFileName(pattern), function (err, files) {
    if (err) {
      cb(err);
    } else {
...
SMBTree.prototype.createFile = function (name, cb) {
  this.spiTree.createFile(consts.normalizeSMBFileName(name), cb);
};
...
SMBTree.prototype.createDirectory = function (name, cb) {
  this.spiTree.createDirectory(consts.normalizeSMBFileName(name), cb);
};
...
SMBTree.prototype.delete = function (name, cb) {
  this.spiTree.delete(consts.normalizeSMBFileName(name), cb);
};
...
SMBTree.prototype.rename = function (oldName, newName, cb) {
  this.spiTree.rename(consts.normalizeSMBFileName(oldName), consts.normalizeSMBFileName(newName), cb);
};
