...
SMBTree.prototype.clearCache = function (cb) {
  var self = this;
  if (self.spiTree) {
    if (self.spiTree.deleteLocalDirectoryRecursive) {
      logger.debug('clearing cache for spi tree');
      self.spiTree.deleteLocalDirectoryRecursive('/', cb);
    } else {
      logger.debug('cannot clear cache because spiTree does not have the expected method');
      cb();
    }
  } else {
