...
SMBTree.prototype.openOrCreate = function (name, createDisposition, openTargetDirectory, cb) {
  ...
  function open(callback) {
    self.spiTree.open(name, function (err, file) {
      ...
      var openAction;
      if (createDisposition === consts.FILE_OVERWRITE
        || createDisposition === consts.FILE_OVERWRITE_IF
        || createDisposition === consts.FILE_SUPERSEDE) {
        openAction = consts.OPEN_ACTION_TRUNCATED;
      } else {
        openAction = consts.OPEN_ACTION_EXISTED;
      }
...
SMBTree.prototype.open = function (name, cb) {
  ...
  this.spiTree.open(utils.normalizeSMBFileName(name), function (err, file) {
    if (err) {
      ...
      var fid = ++SMBTree.fidCounter;
      var result = new SMBFile(file, self, consts.OPEN_ACTION_EXISTED, fid);
      self.files[fid] = result;
      cb(null, result);
    }
  });
...
SMBTree.prototype.createFile = function (name, cb) {
  ...
  this.spiTree.createFile(nname, function (err, file) {
    ...
    var fid = ++SMBTree.fidCounter;
    var result = new SMBFile(file, self, consts.OPEN_ACTION_CREATED, fid);
    self.files[fid] = result;
    cb(null, result);

    self.notifyChangeListeners(consts.FILE_ACTION_ADDED, nname);
...
SMBTree.prototype.delete = function (name, cb) {
  ...
  this.spiTree.delete(nname, function (err) {
    cb(err);
    if (!err) {
      self.notifyChangeListeners(consts.FILE_ACTION_REMOVED, nname);
...
SMBTree.prototype.rename = function (nameOrFile, newName, cb) {
  ...
  this.spiTree.rename(nOldName, nNewName, function (err) {
    ...
    self.notifyChangeListeners(consts.FILE_ACTION_RENAMED, nOldName, nNewName);
  ...
  if (action === consts.FILE_ACTION_RENAMED) {
    ...
    if (listener || listenerNew) {
      if (listener === listenerNew) {
        ...
        listener.cb(consts.FILE_ACTION_RENAMED_OLD_NAME, trimListenerPath(name, listener));
        listenerNew.cb(consts.FILE_ACTION_RENAMED_NEW_NAME, trimListenerPath(newName, listenerNew));
      } else if (listener) {
