 ...
 * @param {Function} cb callback called on completion
 */
File.prototype.rename = function (newName, cb) {
  var self = this;
  this.tree.rename(this.getName(), newName, function (err) {
    if (err) {
      cb(err);
    } else {
      self.onFileNameChanged(newName);
      cb();
    }
  });
};
...
/**
 * Called when the underlying name of this file changed  after a successful rename.
 *
 * @param newName new name
 */
File.prototype.onFileNameChanged = function (newName) {
  this.filePath = Path.join(utils.getParentPath(this.filePath), newName);
