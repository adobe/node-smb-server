...
SMBConnection.prototype.onData = function (data) {
  ...
  while (buf.length >= 4 + consts.SMB_MIN_LENGTH) {
    ...
    if (smbLen > buf.length) {
      logger.debug('encountered partial msg: actual length: %d, expected length: %d, data: %s', buf.length, smbLen, buf.toString('hex'));
  ...
  if (buf.length) {
    logger.debug('encountered partial msg: actual length: %d, expected length: >= %d, data: %s', buf.length, 4 + consts.SMB_MIN_LENGTH, buf.toString('hex'));
    // store partial msg chunk and prepend on next data event
...
SMBConnection.prototype.processMessage = function (msg, cb) {
  ...
  async.eachSeries(msg.commands,
      ...
      if (handler) {
        ...
        handler(msg, cmd.commandId, cmd.params, cmd.data, cmd.paramsOffset, cmd.dataOffset, self, self.server, function (result) {
          if (!result) {
            ...
            msg.processed = true;
            callback();
          } else if (result.status === consts.STATUS_SUCCESS) {
            ...
            callback({
              ...
              message: '\'' + command.toUpperCase() + '\' returned error status ' + consts.STATUS_TO_STRING[result.status] + ' (0x' + result.status.toString(16) + ')'
            });
          }
        });
      } else {
    ...
    function (err) {
      ...
      self.sendResponse(msg, consts.STATUS_SUCCESS, cb);
    }
  );
};
