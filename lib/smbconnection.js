    ...
    function (cmd, callback) {
      ...
      if (handler) {
        ...
        handler(msg, cmd.commandId, cmd.params, cmd.data, cmd.paramsOffset, cmd.dataOffset, self, self.server, function (result) {
          if (!result) {
            // todo fixme: special case (see e.g. 'echo' handler): no further processing required
            msg.processed = true;
            callback();
            ...
            callback({
              ...
              message: '\'' + command + '\' returned error status 0x' + result.status.toString(16)
            });
          }
        });
      } else {
        ...
        callback({
          ...
          message: 'encountered unsupported command 0x' + cmd.commandId.toString(16) + ' \'' + command + '\''
        });
      }
    },
    function (err) {
