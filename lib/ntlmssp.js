...
consts.NTLMSSP_NEGOTIATE_MIN_MSG_LENGTH = 30;
...
function parseNegotiateMessage(buf) {
  ...
  if ((domainOffset && domainOffset !== 32) || workstationOffset !== domainOffset + domainLength || buf.length < workstationOffset + workstationLength) {
    logger.warn('invalid NTLMSSP message: domainLength: %d, domainOffset: %d, domainLength: %d, domainOffset: %d', domainOffset, domainLength, workstationOffset, workstationLength);
    return null;
  }
