...
DefaultAuthenticator.prototype.authenticate = function (challenge, caseInsensitivePassword, caseSensitivePassword, domainName, accountName, cb) {
  ...
  if (caseSensitivePassword.length == auth.ntlm.RESPONSE_LENGTH) {
    // NTLM
    authenticated = auth.validateNTLMResponse(caseSensitivePassword, ntlmHash, challenge);
  } else if (caseSensitivePassword.length >= auth.ntlm2.MIN_RESPONSE_LENGTH) {
    // NTLMv2
    authenticated = auth.validateNTLMv2Response(caseSensitivePassword, ntlmHash, accountName, domainName, challenge);
  } else if (caseInsensitivePassword.length == auth.lm.RESPONSE_LENGTH || caseInsensitivePassword.length == auth.lm2.RESPONSE_LENGTH) {
    // assume LMv2 or LM
    authenticated = auth.validateLMv2Response(caseInsensitivePassword, ntlmHash, accountName, domainName, challenge)
      || auth.validateLMResponse(caseInsensitivePassword, lmHash, challenge);
  } else {
