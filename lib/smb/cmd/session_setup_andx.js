...
function handle(msg, commandId, commandParams, commandData, connection, server, cb) {
  ...
  logger.debug('[%s] maxBufferSize: %d, maxMpxCount: %d, vcNumber: %d, sessionKey: %d, capabilities: %s, accountName: %s, primaryDomain: %s, nativeOS: %s, nativeLanMan: %s',
    consts.COMMAND_TO_STRING[commandId], msg.maxBufferSize, msg.maxMpxCount, msg.vcNumber, msg.sessionKey, msg.capabilities.toString(2),
  ...
  var session = server.setupSession(login, msg.accountName, msg.primaryDomain, msg.caseInsensitivePassword, msg.caseSensitivePassword, function (err, session) {
    if (err) {
    ...
    // don't need login anymore
    server.destroyLogin(login.key);
