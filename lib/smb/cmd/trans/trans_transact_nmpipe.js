...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  logger.debug('[%s] fid: %d [fileName: %s], data: %s', consts.TRANS_SUBCOMMAND_TO_STRING[commandId].toUpperCase(), fid, fileName, commandData.toString('hex'));
  ...
  if (!hdr.firstFrag || !hdr.lastFrag) {
    ...
    result = {
      status: consts.STATUS_INVALID_SMB,
      params: utils.EMPTY_BUFFER,
      data: utils.EMPTY_BUFFER
    };
    process.nextTick(function () { cb(result); });
