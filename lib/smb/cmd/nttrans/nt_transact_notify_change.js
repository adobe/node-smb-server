 ...
 * @param {Function} createResponse - function to invoke to create response
 ...
 */
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, createResponse, connection, server, cb) {
  // decode setup
  ...
  tree.addListener(msg.header.mid, file, function(event, oldName, newName) {
    on(connection, msg, createResponse, event, oldName, newName);
  });
...
function on(connection, msg, createResponse, event, name, newName) {
  ...
  switch (event) {
    ...
    case 'removed':
      ...
      paramsOut = put()
        .word32le(0) // NextEntryOffset
        .word32le(consts.FILE_ACTION_REMOVED) // Action
        .word32le(fileNameBytes.length) // FileNameLength
        .put(fileNameBytes); // FileName
      break;
    case 'modified':
      ...
      paramsOut = put()
        .word32le(0) // NextEntryOffset
        .word32le(consts.FILE_ACTION_MODIFIED) // Action
        .word32le(fileNameBytes.length) // FileNameLength
        .put(fileNameBytes); // FileName
      break;
    case 'renamed':
      // TODO: NextEntryOffset needs to be set
      /*
      fileNameBytes = new Buffer(name, 'utf16le');
      paramsOut = put()
        .word32le(0) // NextEntryOffset
        .word32le(0x00000004) // Action
        .word32le(fileNameBytes.length) // FileNameLength
        .put(fileNameBytes); // FileName
      fileNameBytes = new Buffer(newName, 'utf16le');
      paramsOut.word32le(0) // NextEntryOffset
        .word32le(0x00000005) // Action
        .word32le(fileNameBytes.length) // FileNameLength
        .put(fileNameBytes); // FileName
        */
      break;
  ...
  if (paramsOut) {
    ...
    msg.commands[0].params = result.params;
    msg.commands[0].data = result.data;
    process.nextTick(function () { connection.sendResponse(msg, result.status) });
  }
}
