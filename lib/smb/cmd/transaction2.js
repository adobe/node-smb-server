...
function handle(msg, commandId, commandParams, commandData, connection, server, cb) {
  ...
  if (!subCommand) {
    ...
    callback('encountered invalid subcommand 0x' + cmd.subCommandId.toString(16));
    return;
  }
  ...
  logger.debug('[%s][%s] totalParameterCount: %d, parameterCount: %d, totalDataCount: %d, dataCount: %d, flags: %s', consts.COMMAND_TO_STRING[commandId], subCommand, msg.totalParameterCount, msg.parameterCount, msg.totalDataCount, msg.dataCount, msg.flags.toString(2));
  ...
  if (handler) {
    handler(msg, msg.subCommandId, subParams, subData, connection, server, function (result) {
    ...
    logger.error('encountered unsupported subcommand 0x%s \'%s\'', msg.subCommandId.toString(16), subCommand);
    result = {
      status: consts.STATUS_NOT_IMPLEMENTED,
      params: commandParams,
      data: commandData
