...
function loadSubCmdHandlers() {
  var p = path.join(__dirname, 'trans');
 ...
 * This command serves as the transport for the Transaction Subprotocol Commands.
 * These commands operate on mailslots and named pipes, which are interprocess
 * communication endpoints within the CIFS file system.
 *
...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  if (!msg.setupCount) {
    // https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-rap/
    // https://docs.microsoft.com/en-us/openspecs/windows_protocols/ms-rap/48dd86d2-4092-49a4-9024-308f0ed77520
    logger.error('encountered unsupported protocol: Microsoft Remote Administration Protocol (RAP)');
    result = {
  ...
  var subCommand = SMB.TRANS_SUBCOMMAND_TO_STRING[msg.subCommandId];
  if (!subCommand) {
  ...
  // decode data
  var off = 0;
  msg.name = utils.extractUnicodeBytes(commandData, off);  // not used in transaction
  off += 1;
  ...
  if (msg.parameterCount < msg.totalParameterCount || msg.dataCount < msg.totalDataCount) {
    // todo support transaction_secondary messages (reassembling chunked transaction messages)
    logger.error('chunked transaction messages are not yet supported.');
    result = {
      status: ntstatus.STATUS_NOT_IMPLEMENTED,
      params: utils.EMPTY_BUFFER,
      data: utils.EMPTY_BUFFER
  ...
  if (handler) {
    handler(msg, msg.subCommandId, subParams, subData, msg.parameterOffset, msg.dataOffset, connection, server, function (result) {
      ...
      // transaction response have a params length of 20 (10 words) plus length of setup
