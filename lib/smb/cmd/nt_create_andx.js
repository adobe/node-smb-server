...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  function getFile(callback) {
    ...
    server.getTree(msg.header.tid).openOrCreate(msg.fileName, msg.createDisposition, msg.createOptions & common.FILE_DIRECTORY_FILE, callback);
  }
  ...
  function buildResult(file, callback) {
    ...
    out.word8(commandParams.readUInt8(0)) // andX next cmd id
      ...
      .word16le(file.getTree().getShare().isNamedPipe() ? consts.FILE_TYPE_MESSAGEMODEPIPE : consts.FILE_TYPE_DISK);  // ResourceType
