...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  function buildResult(file, callback) {
    ...
    if (server.protocol === 'SMB' && msg.flags & NT_CREATE_REQUEST_EXTENDED_RESPONSE) {
      // MS-SMB v1.0
      out.word16le(NO_EAS | NO_SUBSTREAMS | NO_REPARSETAG)  // FileStatusFlags
        ...
        .word64le(0)  // FileId
        .word32le(consts.GENERIC_ALL)  // MaximalAccessRights
        .word32le(0);  // GuestMaximalAccessRights
    } else {
      // CIFS
