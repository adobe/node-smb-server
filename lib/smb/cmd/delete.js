...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  var inclHidden = !!(searchAttributes & consts.ATTR_HIDDEN);
  var inclSystem = !!(searchAttributes & consts.ATTR_SYSTEM);

  logger.debug('[%s] searchAttributes: %s, fileName: %s', consts.COMMAND_TO_STRING[commandId].toUpperCase(), searchAttributes.toString(2), fileName);
  ...
  tree.list(fileName, function (err, files) {
    ...
    async.each(
      ...
      function (err) {
        cb({
          status: err ? consts.STATUS_UNSUCCESSFUL : consts.STATUS_SUCCESS,
