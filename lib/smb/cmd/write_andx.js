...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  // data to be written
  var data = msg.buf.slice(msg.dataOffset, msg.dataOffset + msg.dataLength);

  var tree = server.getTree(msg.header.tid);
  var fileName = tree && tree.getFile(msg.fid) && tree.getFile(msg.fid).getName() || null;

  logger.debug('[%s] fid: %d [fileName: %s], offset: %d, timeout: %d, remaining: %d, dataLength: %d, dataOffset: %d', consts.COMMAND_TO_STRING[commandId].toUpperCase(), msg.fid, fileName, msg.offset, msg.timeout, msg.remaining, msg.dataLength, msg.dataOffset);
