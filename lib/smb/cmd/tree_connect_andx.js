...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  logger.debug('[%s] flags: %s, password: %s, service: %s, path: %s', consts.COMMAND_TO_STRING[commandId].toUpperCase(), msg.flags.toString(2), msg.password.toString('hex'), msg.service, msg.path);
  ...
  server.connectTree(session, shareName, shareLevelPassword, function (err, tree) {
    ...
    var service = tree.getShare().isNamedPipe() ? consts.SERVICE_NAMEDPIPE : consts.SERVICE_DISKSHARE;
    if (msg.service !== consts.SERVICE_ANY && msg.service !== service) {
      result = {
    ...
    if (msg.flags & TREE_CONNECT_ANDX_EXTENDED_RESPONSE) {
      // MS-SMB v1.0
      out.word32le(consts.TREE_ACCESS_ALL)  // MaximalShareAccessRights
        .word32le(consts.TREE_ACCESS_READONLY);  // GuestMaximalShareAccessRights
    }
