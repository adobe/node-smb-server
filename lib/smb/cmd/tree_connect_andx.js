...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  logger.debug('[%s] flags: %s, password: %s, service: %s, path: %s', consts.COMMAND_TO_STRING[commandId], msg.flags.toString(16), msg.password.toString('hex'), msg.service, msg.path);

  var shareName = msg.path.substring(msg.path.lastIndexOf('\\') + 1);
  var shareLevelPassword = msg.password.equals(new Buffer([ 0x00 ])) ? null : msg.password;
