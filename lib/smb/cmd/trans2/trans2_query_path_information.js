...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  async.waterfall([ getFile, buildResult, closeFile ], function (err, result) {
    if (err) {
      logger.error('[%s] informationLevel: %s, fileName: %s failed', consts.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(), consts.QUERY_PATH_INFORMATION_LEVEL_TO_STRING[informationLevel], fileName, err);
      logger.error(err);
      cb({
