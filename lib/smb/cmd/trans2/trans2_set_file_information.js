...
function handle(msg, commandId, commandParams, commandData, commandParamsOffset, commandDataOffset, connection, server, cb) {
  ...
  switch (informationLevel) {
    ...
    case consts.SET_FILE_DISPOSITION_INFORMATION:
      ...
      if (deletePending) {
        logger.debug('%s, fid: %d [fileName: %s], deletePending: %d', consts.SET_INFORMATION_LEVEL_TO_STRING[informationLevel], fid, fileName, deletePending);
    ...
    case consts.SET_FILE_ALLOCATION_INFORMATION:
    ...
    case consts.SET_FILE_RENAME_INFORMATION:
      ...
      logger.debug('%s, fid: %d [fileName: %s], replaceIfExists: %d, rootDirectoryHandle: %d, fileNameLength: %d, targetName: %s', consts.SET_INFORMATION_LEVEL_TO_STRING[informationLevel], fid, fileName, replaceIfExists, rootDirectory, fileNameLength, targetName);
    ...
    default:
      logger.error('[%s] encountered unsupported or unknown informationLevel %s, fid: %d [fileName: %s]', consts.TRANS2_SUBCOMMAND_TO_STRING[commandId].toUpperCase(), consts.SET_INFORMATION_LEVEL_TO_STRING[informationLevel] || '0x' + informationLevel.toString(16), fid, fileName);
      result = {
