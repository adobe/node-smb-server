...
function decode(buf) {
  ...
  if (utils.isAndXCommand(cmdId)) {
    msg.chainedCommands = [];
    decodeChainedCommands(buf, header.commandId, params, data, msg.chainedCommands);
  }

  return msg;
}

function decodeChainedCommands(buf, commandId, params, data, chainedCommands) {
  ...
  if (nextCmdId != 0xff && offset) {
    ...
    // recurse
    decodeChainedCommands(buf, nextCmdId, nextParams, nextData, chainedCommands);
  }
}
...
function encode(msg) {
  ...
  if (utils.isAndXCommand(msg.header.commandId)) {
    ...
    for (var i = 0; i < msg.chainedCommands.length; i++) {
      var cmd = msg.chainedCommands[i];
      // next params/data block offset
      offset += (1 + cmd.params.length + 2 + cmd.data.length);
      // wordCount
      out.word8(cmd.params.length / 2);
      // andX header
      if (i === msg.chainedCommands.length - 1) {
        ...
        // next andX command
        cmd.params.writeUInt8(msg.chainedCommands[i + 1].commandId, 0);
        // andX reserved
        cmd.params.writeUInt8(0x0, 1);
        // andX offset
