...
function encode(msg) {
  ...
  // body (params, data)
  if (msg.header.status !== consts.STATUS_SUCCESS) {
    // error message
  ...
  } else if (!utils.isAndXCommand(msg.header.commandId)) {
    ...
    for (var i = 0; i < msg.commands.length; i++) {
      ...
      // wordCount
      if (msg.header.commandId === consts.STRING_TO_COMMAND['nt_create_andx']
