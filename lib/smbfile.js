 ...
 *
 * @param {File} spiFile
 * @param {Number} [openAction = OPEN_ACTION_EXISTED]
 * @param {Number} [fid = 0]
 * @constructor
 */
function SMBFile(spiFile, openAction, fid) {
  ...
  } else if (spiFile.isHidden()) {
    this.attributes |= consts.ATTR_HIDDEN;
  } else if (spiFile.isFile()) {
    this.attributes |= consts.ATTR_NORMAL;
  }
  if (spiFile.isReadOnly()) {
    this.attributes |= consts.ATTR_READ_ONLY;
  }
...
/**
 * Called when the name of <i>this</i> file instance changed after a preceding rename.
 *
 * @param newName new name
 */
SMBFile.prototype.onFileNameChanged = function (newName) {
  logger.debug('[fid=%d] file.onFileNameChanged %s newName=%s', this.fid, this.getPath(), newName);

  this.spiFile.onFileNameChanged(newName);
};
...
SMBFile.prototype.close = function (cb) {
  logger.debug('[fid=%d] file.close %s', this.fid, this.getPath());
  var self = this;
  ...
  async.series([
    ...
    function (callback) {
      if (self.deleteOnClose) {
        // use Tree.delete instead of SMBFile.delete:
        // the current File instance could be in an inconsistent state after
        // a preceding rename from e.g. 'test.doc' to '.tmptest.doc'
        //self.delete(callback);
        if (self.isFile()) {
          self.spiFile.getTree().delete(self.getPath(), callback);
        } else {
          self.spiFile.getTree().deleteDirectory(self.getPath(), callback);
        }
      } else {
        callback();
      }
