 ...
 */
consts.SMB_INFO_STANDARD = 0x0001;  // Return creation, access, and last write timestamps, size and file attributes along with the file name.
consts.SMB_INFO_QUERY_EA_SIZE = 0x0002; // Return the SMB_INFO_STANDARD data along with the size of a file's extended attributes (EAs).
consts.SMB_INFO_QUERY_EAS_FROM_LIST = 0x0003; // Return the SMB_INFO_QUERY_EA_SIZE data along with a specific list of a file's EAs. The requested EAs are provided in the Trans2_Data block of the request.
consts.SMB_FIND_FILE_DIRECTORY_INFO = 0x0101; // Return 64-bit format versions of: creation, access, last write, and last attribute change timestamps; size. In addition, return extended file attributes and file name.
consts.SMB_FIND_FILE_FULL_DIRECTORY_INFO = 0x0102;  // Returns the SMB_FIND_FILE_DIRECTORY_INFO data along with the size of a file's EAs.
consts.SMB_FIND_FILE_NAMES_INFO = 0x0103; // Returns the name(s) of the file(s).
consts.SMB_FIND_FILE_BOTH_DIRECTORY_INFO = 0x0104;  // Returns a combination of the data from SMB_FIND_FILE_FULL_DIRECTORY_INFO and SMB_FIND_FILE_NAMES_INFO.

/**
 * SET information level codes
 */
consts.SMB_INFO_STANDARD = 0x0001;  // Set creation, access, and last write timestamps.
consts.SMB_INFO_SET_EAS = 0x0002; // Set a specific list of extended attributes (EAs).
consts.SMB_SET_FILE_BASIC_INFO = 0x0101;  // Set 64-bit create, access, write, and change timestamps along with extended file attributes. Not supported for TRANS2_SET_PATH_INFORMATION (section 2.2.6.7).
consts.SMB_SET_FILE_DISPOSITION_INFO = 0x0102;  // Set whether or not the file is marked for deletion. Not supported for TRANS2_SET_PATH_INFORMATION (section 2.2.6.7).
consts.SMB_SET_FILE_ALLOCATION_INFO = 0x0103; // Set file allocation size. Not supported for TRANS2_SET_PATH_INFORMATION (section 2.2.6.7).
consts.SMB_SET_FILE_END_OF_FILE_INFO = 0x0104;  // Set file EOF offset. Not supported for TRANS2_SET_PATH_INFORMATION (section 2.2.6.7).

/**
 * FIND flags
 */
consts.SMB_FIND_CLOSE_AFTER_REQUEST = 0x0001; // Close the search after this request.
consts.SMB_FIND_CLOSE_AT_EOS = 0x0002;  // Close search when end of search is reached.
consts.SMB_FIND_RETURN_RESUME_KEYS = 0x0004;  // Return resume keys for each entry found.
consts.SMB_FIND_CONTINUE_FROM_LAST = 0x0008;  // Continue search from previous ending place.
consts.SMB_FIND_WITH_BACKUP_INTENT = 0x0010;  // Find with backup intent.
 ...
 * Create Disposition
 */
consts.FILE_SUPERSEDE = 0x00; // If the file already exists, replace it with the given file. If it does not, create the given file.
consts.FILE_CREATE = 0x01;  // If the file already exists, fail the request and do not create or open the given file. If it does not, create the given file.
consts.FILE_OPEN = 0x02;  // If the file already exists, open it instead of creating a new file. If it does not, fail the request and do not create a new file.
consts.FILE_OPEN_IF = 0x03; // If the file already exists, open it. If it does not, create the given file.
consts.FILE_OVERWRITE = 0x04; // If the file already exists, open it and overwrite it. If it does not, fail the request.
consts.FILE_OVERWRITE_IF = 0x05;  // If the file already exists, open it and overwrite it. If it does not, create the given file.
