...
function handleRequest(msgBuf, connection, server, cb) {
  ...
  function processMsg(msg, callback) {
    ...
    if (handler) {
      // process command
      handler(msg, cmd.commandId, msg.body, relatedCtx, connection, server, function (result) {
        if (!result) {
      ...
      // no handler found
      logger.error('encountered unsupported command 0x' + cmd.commandId.toString(16) + ' \'' + command.toUpperCase() + '\'');
