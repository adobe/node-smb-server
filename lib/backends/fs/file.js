...
var FSFile = function (filePath, stats) {
  logger.debug('[fs] open %s', filePath);
...
FSFile.prototype.read = function (buffer, offset, length, position, cb) {
  logger.debug('[fs] read %s offset=%d, length=%d, position=%d', this.filePath, offset, length, position);
...
FSFile.prototype.write = function (data, position, cb) {
  logger.debug('[fs] write %s data.length=%d, position=%d', this.filePath, data.length, position);
  var callback = SMBError.systemToSMBErrorTranslator(cb);
  var self = this;
  fs.open(this.filePath, 'r+', function (err, fd) {
    if (err) {
...
FSFile.prototype.setLength = function (length, cb) {
  logger.debug('[fs] setLength %s length=%d', this.filePath, length);
  var callback = SMBError.systemToSMBErrorTranslator(cb);
  var self = this;
...
FSFile.prototype.delete = function (cb) {
  logger.debug('[fs] delete %s', this.filePath);
  if (this.isDirectory()) {
    fs.rmdir(this.filePath, SMBError.systemToSMBErrorTranslator(cb));
  } else {
...
FSFile.prototype.flush = function (cb) {
  logger.debug('[fs] flush %s', this.filePath);
...
FSFile.prototype.close = function (cb) {
  logger.debug('[fs] close %s', this.filePath);
  // there's nothing to do here
  process.nextTick(function () { cb(); });
};
