...
/*
  ...
  async.waterfall([
      function (done) {
        if (self.fd && write && !self.isWrite) {
          // previously opened in readonly mode, need to close first in order to open in readwrite mode
          perflog.debug('%s File.getDescriptor.fs.close', self.getPath());
          var fd = self.fd;
          self.fd = null;
          fs.close(fd, SMBError.systemToSMBErrorTranslator(done));
        } else {
          done();
        }
      },
      function (done) {
        ...
        fs.open(self.realPath, write ? 'r+' : 'r', SMBError.systemToSMBErrorTranslator(done));
      },
      function (fd, done) {
        if (fd != self.fd) {
          self.fd = fd;
...
FSFile.prototype.read = function (buffer, offset, length, position, cb) {
  ...
  async.waterfall([
      function (done) {
        self.getDescriptor(false, done);
      },
      function (fd, done) {
        perflog.debug('%s File.read.fs.read %d', self.filePath, length);
        fs.read(fd, buffer, offset, length, position, SMBError.systemToSMBErrorTranslator(done));
      }
    ],
    cb
  );
...
FSFile.prototype.write = function (data, position, cb) {
  ...
  async.waterfall([
      function (done) {
        self.getDescriptor(true, done);
      },
      function (fd, done) {
        perflog.debug('%s File.write.fs.write %d', self.getPath(), data.length);
        fs.write(fd, data, 0, data.length, position, SMBError.systemToSMBErrorTranslator(done));
      }
    ],
    cb
  );
...
FSFile.prototype.setLength = function (length, cb) {
  ...
  async.series([
      ...
      function (done) {
        ...
        fs.truncate(self.realPath, length, SMBError.systemToSMBErrorTranslator(done));
      },
      function (done) {
        // update stats
  ...
  async.series([
      ...
      function (done) {
        ...
        if (self.isDirectory()) {
          ...
          fs.rmdir(self.realPath, SMBError.systemToSMBErrorTranslator(done));
        } else {
          perflog.debug('%s File.delete.fs.unlink', self.getPath());
          fs.unlink(self.realPath, SMBError.systemToSMBErrorTranslator(done));
        }
      }
    ],
    cb
  ...
  if (this.fd) {
    async.series([
        function (done) {
          ...
          fs.fsync(self.realPath, SMBError.systemToSMBErrorTranslator(done));
        },
        function (done) {
          // update stats
          self.refreshStats(done);
