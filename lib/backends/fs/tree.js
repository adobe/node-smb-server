...
FSTree.prototype.list = function (pattern, cb) {
  ...
  if (filter == '*') {
    ...
    fs.readdir(realParentPath, function (err, files) {
      ...
      var matchingNames = filter === '*' ? files : files.filter(function (fileName) {
        return self.unicodeEquals(fileName, filter);
      });

      async.reduce(matchingNames, [],
          function (memo, name, callback) {
            self.createFileInstance(Path.join(parentPath, name), self, function (err, file) {
              if (err) {
                logger.warn('[%s] tree.list %s: unexpected error while attempting to include %s in list', self.share.config.backend, pattern, name, err);
                callback(null, memo);
              } else {
                memo.push(file);
                callback(null, memo);
    ...
    self.exists(pattern, function (err, exists) {
      if (err) {
        ...
        self.createFileInstance(pattern, self, function (err, file) {
          if (err) {
            cb(err);
          } else {
            cb(null, [file]);
