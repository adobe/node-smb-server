...
FSTree.prototype.open = function (name, cb) {
  ...
  var realPath = Path.join(this.share.path, name);
  var self = this;
  fs.stat(realPath, function (err, stats) {
    if (err) {
      callback(err);
    } else {
      cb(null, new FSFile(name, stats, self));
    }
  });
};
  ...
  fs.readdir(realParentPath, function (err, files) {
    ...
    async.map(matchingNames,
      function (name, callback) {
        fs.stat(Path.join(realParentPath, name), function (err, stats) {
          if (err) {
            callback(err);
          } else {
            callback(null, new FSFile(Path.join(parentPath, name), stats, self));
          }
        });
      },
