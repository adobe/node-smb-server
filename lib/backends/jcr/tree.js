...
var TEMP_FILE_PATTERNS = [
  ...
  /\.tmp$/, // Illustrator: on save, creates one or more *.tmp files, renames them to original file name
  // OS X
...
JCRTree.prototype.createFileInstancesFromContent = function (content, path, namePattern, cb) {
  ...
  this.share.parseContentChildEntries(content, function (childName, childContent) {
    ...
    if (namePattern === '*' || utils.unicodeEquals(childName, namePattern)) {
...
JCRTree.prototype.exists = function (name, cb) {
  ...
  // doesn't exist. we end up in a redirect loop. instead of requesting the resource
  // itself, request its content url
...
JCRTree.prototype.list = function (pattern, cb) {
  ...
  function getRemoteContent() {
    ...
    function getContent(done) {
      self.share.getContent(path, true, function (err, content) {
        if (err) {
          done(err);
        } else {
          done(null, content);
    ...
    function createEntries(content, done) {
      self.createFileInstancesFromContent(content, path, pattern, function (err, files) {
        if (err) {
          done(err);
        } else {
          done(null, _.merge(results, files));
        }
      });
    }
  ...
  if (this.tempFilesTree) {
    this.tempFilesTree.list(pattern, function (err, tmpFiles) {
      if (err) {
        if (err.status !== ntstatus.STATUS_NO_SUCH_FILE) {
          cb(err);
          return;
        }
        tmpFiles = [];
