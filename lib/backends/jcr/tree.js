...
JCRTree.prototype.fetchContent = function (name, depth, cb) {
  ...
  var url = 'http://' + this.share.host + ':' + this.share.port + this.share.path + name + '.' + depth + '.json';
  var options = {
    url: url,
  ...
  this.tempFilesTree.list(pattern, function (err, tmpFiles) {
    ...
    function createEntries(content, done) {
      ...
      _.forOwn(directories, function(entry, name) {
        result.push(self.createFileInstance(name, entry, 0));
      });
        ...
        function (p, callback) {
          ...
          self.fetchFileLength(p, function (err, length) {
            if (err) {
              ...
              result.push(self.createFileInstance(p, files[p], length));
              callback();
            }
          });
        },
