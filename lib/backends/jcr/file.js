  ...
  if (this.tree.share.path === '/content/dam') {
    ...
    request(options, function (err, resp, body) {
      if (err) {
        ...
        logger.error('failed to delete %s [statusCode: %d]', self.filePath, resp.statusCode, err);
        cb(new SMBError(consts.STATUS_UNSUCCESSFUL));
      } else {
        // succeeded
        if (self.fsFile) {
...
JCRFile.prototype.flush = function (cb) {
  if (!this.fsFile) {
    logger.warning('flush %s without changes...?!', this.filePath);
    ...
    if (self.tree.share.path === '/content/dam') {
      ...
      fs.createReadStream(localFile.filePath).pipe(
        request(options)
          .on('response', function (resp) {
            if (resp.statusCode !== 200) {
              logger.error('failed to spool %s to %s [statusCode: %d]', localFile.filePath, self.filePath, resp.statusCode);
              failed = true;
            }
          })
          .on('error', function (err) {
        ...
        // todo update length, lastmodified etc
        cb(failed ? new SMBError(consts.STATUS_UNSUCCESSFUL) : null);
      });
    } else {
      // todo use sling/davex api
      process.nextTick(function () { cb(new SMBError(consts.STATUS_NOT_IMPLEMENTED)); });
