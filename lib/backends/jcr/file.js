...
JCRFile.createInstance = function (filePath, tree, content, fileLength, cb) {
  ...
  function getContent(callback) {
    if (content) {
      ...
      tree.fetchContent(filePath, 1, function (err, content) {
        if (content) {
          callback(err, content);
        } else {
          callback(err || new SMBError(ntstatus.STATUS_NO_SUCH_FILE));
        }
      });
    }
  }
  ...
  function getFileLength(content, callback) {
    ...
    } else if (!tree.isFilePrimaryType(content[JCR.JCR_PRIMARYTYPE])) {
      // folder has length 0
