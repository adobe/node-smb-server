  ...
  stream.on('finish', function () {
    if (failed) {
      fs.unlink(tmpFilePath, function (err) {
        cb('failed to spool ' + self.filePath + ' to ' + tmpFilePath + ': ' + err.message);
      });
    } else {
      fs.stat(tmpFilePath, function (err, stats) {
        if (err) {
          ...
          logger.debug('[%s] spooled %s to %s (%d bytes)', self.tree.share.config.backend, self.filePath, tmpFilePath, stats.size);
...
JCRFile.prototype.flush = function (cb) {
  ...
  this.ensureGotLocalCopy(function (err, localFilePath) {
    ...
    var url = 'http://' + self.tree.share.host + ':' + self.tree.share.port + self.tree.share.path + self.filePath;
...
JCRFile.prototype.close = function (cb) {
  ...
  function cleanup(callback) {
    // delete local tmp file
    fs.unlink(self.localFilePath, callback);
    self.localFilePath = null;
  }

  async.series([ autoFlush, cleanup ], cb);
};
 ...
 */
JCRFile.prototype.onFileNameChanged = function (newName) {
  // call base class method
  File.prototype.onFileNameChanged.call(this, newName);
  if (this.localFilePath) {
    var newLocalFilePath = Path.join(Path.dirname(this.localFilePath), newName);
    fs.renameSync(this.localFilePath, newLocalFilePath);
    this.localFilePath = newLocalFilePath;
  }
};
