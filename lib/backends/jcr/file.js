...
/**
 * Called when the length of the underlying file changed
 *
 * @param fileLength
 */
JCRFile.prototype.onSizeChanged = function (fileLength) {
  this.fileLength = fileLength;
  // update (potentially cached) content representation
  ...
  this.ensureGotLocalCopy(function (err, localFilePath) {
    ...
    fs.truncate(localFilePath, length, function (err) {
      if (err) {
        ...
        // update length and lastModified
        self.onSizeChanged(length);
        self.setLastModified(new Date().getTime());
        self.dirty = true;
        // forced sync in order to avoid InDesign file corruption:
