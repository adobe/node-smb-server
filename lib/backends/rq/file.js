 ...
 * @private
 * @this {RQFile}
 * @param {String} openFile normalized file path
...
RQFile.prototype.cacheFile = function (cb) {
  ...
  var syncReadOnlyStatus = function (sourceFile, targetFile, callback) {
    if (targetFile.isFile() && sourceFile.isReadOnly() != targetFile.isReadOnly()) {
      logger.debug('target file readonly is %s but source is not. resetting source readonly status', sourceFile.isReadOnly());
      targetFile.setReadOnly(sourceFile.isReadOnly(), callback);
    }
  };
                  ...
                  if (err) {
                    ...
                    self.remote.open(self.tree.remoteEncodePath(self.filePath), function (err, remoteFile) {
                      if (err) {
                        ...
                        syncReadOnlyStatus(remoteFile, localFile, function (err) {
                          if (err) {
                            self.tree.handleErr(cb, err);
                          } else {
          ...
          if (err) {
            ...
            self.local.open(self.filePath, function (err, file) {
              if (err) {
                ...
                syncReadOnlyStatus(self.openFile, file, function (err) {
                  if (err) {
                    self.tree.handleErr(cb, err);
                  } else {
                    self.openFile = file;
