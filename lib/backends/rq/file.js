  ...
  if (self.isFile()) {
    self.work.exists(filePath, function (err, exists) {
      if (err) {
        ...
        logger.debug('created file for %s exists, retrieving', filePath);
        self.work.createFileInstance(filePath, self.work, function (err, createdFile) {
          if (err) {
            cb(err);
          } else {
      ...
      } else if (self.tree.isTempFileName(filePath)) {
        // don't create a file for temp files
        logger.debug('%s is a temp file, no created file will be made', filePath);
        cb(null, self);
      } else {
        logger.debug('created file for %s does not exist, creating now', filePath);
  ...
  if (self.localFile) {
    self.processCreatedFile(function (err, createdFile) {
      if (err) {
        ...
        var created = createdFile.created();
        var lastModified = self.lastModified();
        var modifiedDiff = lastModified - created;
        self.work.exists(self.tree.getCreateFileName(self.filePath), function (err, exists) {
          if (err) {
            cb(err);
          } else {
            logger.debug('%s: canDelete result based on creation file exists value of %s and modified diff of %d (%d - %d). threshold is %d', self.filePath, exists, modifiedDiff, lastModified, created, self.tree.share.config.modifiedThreshold);
            // a file can only be safely deleted if it hasn't been modified locally and wasn't created locally
            cb(null, (modifiedDiff <= self.tree.share.config.modifiedThreshold) && !exists);
          }
        });
      } else {
        logger.debug('%s is a directory, so it can be deleted', self.filePath);
        // the file is a directory
        cb(null, true);
      }
    });
  } else {
    logger.debug('%s has not been cached locally, so it can be deleted', self.filePath);
    cb(null, true);
  }
};
...
RQFile.prototype.created = function () {
  if (this.createdFile) {
    var created = this.createdFile.created();
    var modified = this.lastModified();
    // it might be that the file was cached but the created file doesn't exist. in this case assume that the
    // file is newly cached and use the modified date instead of the created date.
    if (modified < created) {
      created = modified;
    }
    return created;
