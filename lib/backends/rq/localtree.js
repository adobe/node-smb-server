 ...
 * @param {string} filePath The path to the work file to which to write.
 * @param {File} local The local file whose information will be used in the cache info file.
 * @param {boolean} isCreated The created flag to write to the file.
 * @param {File} remote The remote file whose information will be used in the cache info file.
 ...
 * @private
 */
function _writeWorkData(filePath, local, isCreated, remote, cb) {
  var self = this;
  logger.debug('_writeWorkData: entering with filePath=%s, local=%s, isCreated=%s, remote=%s', filePath, local ? local.getPath() : '', isCreated, remote ? remote.getPath() : '');
  self.source.open(filePath, function (err, workFile) {
    if (err) {
      logger.error('unable to open work file for writing %s', filePath, err);
      cb(err);
    } else {
      var writeData = JSON.stringify(RQLocalFile.getCacheInfo(local, remote, isCreated));
 ...
 * @private
 */
function _createWorkFile(targetFile, created, remoteFile, cb) {
  ...
  function _createFile(path) {
    self.source.createFile(path, function (err) {
      if (err) {
        cb(err);
      } else {
        logger.debug('%s work file created', path);
        _writeWorkData.call(self, path, targetFile, created, remoteFile, function (err) {
          if (err) {
            cb(err);
          } else {
            _openFile(path);
...
RQLocalTree.prototype.download = function (remote, name, cb) {
  ...
  self.setDownloading(name, true, function (err) {
    if (err) {
      ...
      remote.open(name, function (err, remoteFile) {
        if (err) {
          cb(err);
        } else {
  ...
  _deleteWork(function () {
    self.source.open(name, function (err, localFile) {
      if (err) {
        ...
        _createWorkFile.call(self, localFile, false, remoteFile, function (err, workFile) {
...
RQLocalTree.prototype.createFromSource = function (source, remote, isCreated, cb) {
  var self = this;
  _createWorkFile.call(self, source, isCreated, remote, function (err, workFile) {
    if (err) {
      cb(err);
    } else {
 ...
 * @param {boolean} newCreated If truthy, the new file will be flagged as a newly created file.
 * @param {Function} cb callback called on completion
 * @param {SMBError} cb.error error (non-null if an error occurred)
 */
RQLocalTree.prototype.renameExt = function (oldName, newName, newCreated, cb) {
  var self = this;
  var newCreated = newCreated ? true : false;
  ...
  function _deleteCacheInfo(deleteName, doDelete, delCb) {
    self.cacheInfoExists(deleteName, function (err, exists) {
      if (err) {
        ...
        logger.warn('%s rename involved cache info file already exists, removing', deleteName);
          ...
          if (!newExisted) {
            ...
            self.source.open(newName, function (err, renamed) {
              if (err) {
                ...
                _createWorkFile.call(self, renamed, newCreated, null, function (createErr) {
            ...
            logger.debug('%s work file for target already existed, using existing', newName);
            cb();
          }
        });
      });
...
RQLocalTree.prototype.rename = function (oldName, newName, cb) {
  this.renameExt(oldName, newName, true, cb);
};
