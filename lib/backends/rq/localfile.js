...
RQLocalFile.createInstance = function (sourceFile, infoFile, tree, cb) {
  if (infoFile) {
    ...
    var buffer = new Array(infoFile.size());
    infoFile.read(buffer, 0, infoFile.size(), 0, function (err) {
      if (err) {
        ...
        var cacheInfo;
        try {
          cacheInfo = JSON.parse(buffer.join(''));
        } catch (e) {
 ...
 */
RQLocalFile.getCacheInfo = function (local, remote, created) {
  var writeData = {
    local: {
  ...
  if (remote) {
    writeData['remote'] = {
      ...
      lastChanged: remote.lastChanged(),
...
RQLocalFile.prototype.canDelete = function (cb) {
  ...
  if (self.isDirectory()) {
    ...
    cb(null, self.source.lastModified() == lastModifiedAtCache && !self.isCreatedLocally() && self.getDownloadedRemoteModifiedDate());
  }
};
...
function _getDate(property, currValue) {
  var date = currValue;
  if (this.data.remote[property] && !this.isCreatedLocally()) {
    if (currValue == this.data.local[property]) {
      date = this.data.remote[property];
    }
  }
...
RQLocalFile.prototype.lastModified = function () {
  return _getDate.call(this, 'lastModified', this.source.lastModified());
};
...
RQLocalFile.prototype.lastChanged = function () {
  return _getDate.call(this, 'lastChanged', this.source.lastChanged());
};
...
RQLocalFile.prototype.created = function () {
  return _getDate.call(this, 'created', this.source.created());
};
...
RQLocalFile.prototype.lastAccessed = function () {
  return _getDate.call(this, 'lastAccessed', this.source.lastAccessed());
};
