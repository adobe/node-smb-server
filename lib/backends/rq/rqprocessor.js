...
RQProcessor.prototype.start = function (config) {
    ...
    var processItemMethod = function (item, method, cb) {
        ...
        var url = item.remotePrefix + Path.join(item.path, item.name);
        var path = Path.join(item.localPrefix, item.path, item.name);
                ...
                if (err) {
                    ...
                    self.rq.removeRequest(item.path, item.name, function (err) {
                        if (err) {
                            ...
                            RQFile.deleteWorkFiles(item.path, self.tree, function (err) {
                                if (err) {
                                    logger.error('unable to delete work files for local file %s', item.path, err);
                                }
