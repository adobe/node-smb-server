...
RQTree.prototype.deleteLocalDirectoryRecursive = function (name, cb) {
  ...
  if (name == '/') {
    ...
    self.handleErr(cb, 'recursive deletion of root directories is forbidden');
  } else {
      ...
      if (exists) {
        ...
        self.local.rename(oldName, newName, function (err) {
          if (err) {
            ...
            self.local.open(newName, function (err, file) {
              function processCb(err) {
                file.close(function (closeErr) {
                  if (closeErr) {
                    logger.error('unable to close file %s after rename', newName);
                  }
                  self.handleErr(cb, err);
                });
              }
        ...
        logger.error('attempting to rename temp file %s, which does not exist', oldName);
