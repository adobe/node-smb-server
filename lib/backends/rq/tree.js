...
var RQTree = function (share, remote, options) {
  ...
  this.remote = remote;
  this.local = new RQLocalTree(share, options.localTree, this);
  this.share = share;
 ...
 * @param {boolean} newCreated If truthy, the target file will be flagged as a newly created file.
 * @param {function} cb Will be invoked when the operation is complete.
 * @param {string|Error} cb.err Will be truthy if there were errors.
 */
RQTree.prototype.renameLocal = function (oldName, newName, newCreated, cb) {
  this.local.renameExt(oldName, newName, newCreated, cb);
};
...
RQTree.prototype.isLocalDirectory = function (name, cb) {
  this.isLocalDirectoryOpen(name, cb);
};

RQTree.prototype.isLocalDirectoryOpen = function (name, cb) {
  var self = this;
...
RQTree.prototype.refreshWorkFiles = function (filePath, cb) {
  ...
  self.remote.open(filePath, function (err, remote) {
    if (err) {
      self.handleErr(cb, err);
    } else {
...
function _getReplaceInfo(oldName, newName, cb) {
  ...
  function _isDirectory(callback) {
    self.open(oldName, function (err, file) {
      if (err) {
        callback(err);
      } else {
        file.close(function (err) {
  ...
  function _getOldInfo(callback) {
    self.existsLocal(oldName, function (err, oldExists) {
      if (err) {
        callback(err);
      } else {
        info['oldExistsLocal'] = oldExists;
        callback();
      }
    });
  }
    ...
    if (info.isDir) {
      ...
      self.isQueuedFor(newName, ['DELETE', 'POST'], function (err, queued) {
        if (err) {
          ...
          self.exists(newName, function (err, newExists) {
            if (err) {
              callback(err);
            } else {
              info['newQueued'] = queued;
              info['newExists'] = newExists;
              if (newExists) {
                self.existsLocal(newName, function (err, newExistsLocal) {
                  if (err) {
                    callback(err);
                  } else {
  ...
  async.series([_isDirectory, _getOldInfo, _getNewInfo], function (err) {
    if (err) {
      cb(err);
    } else {
      cb(null, info);
  ...
  _getReplaceInfo.call(this, oldName, newName, function (err, info) {
    ...
    if (err) {
      ...
      if (info.oldExistsLocal) {
        self.renameLocal(oldName, newName, (!info.newQueued), function (err) {
          if (err) {
            self.handleErr(cb, err);
          } else {
