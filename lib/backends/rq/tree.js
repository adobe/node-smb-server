...
var RQTree = function (share, remote, options) {
  ...
  if (!options.workTree) {
    options.workTree = new FSTree(new FSShare('rqwork', share.config.local));
  }
...
function _existsLocally(name, cb) {
  ...
  self.local.exists(name, function (err, localExists) {
    if (err) {
      ...
      if (localExists) {
        self.work.isDownloading(name, function (err, isDownloading) {
          if (err) {
            cb(err);
          } else {
...
function _deleteLocalFile(tree, localFile, cb) {
  ...
  // do so
  tree.canDelete(localFile, function (err, canDelete) {
    if (err) {
      cb(err);
    } else if (canDelete) {
      ...
      if (localFile.isDirectory()) {
        ...
        tree.local.delete(localFile.getPath(), function (err) {
          if (err) {
            cb(err);
          } else {
            logger.debug('local file %s was deleted locally. preparing to delete work files', localFile.getPath());
            tree.work.delete(localFile.getPath(), function (err) {
              if (err) {
                cb(err);
              } else {
...
function _preserveCacheFile(path) {
  if (this.share.config.preserveCacheFiles) {
    var i;
    for (i = 0; i < this.share.config.preserveCacheFiles.length; i++) {
      if (path == Path.join(Path.sep, this.share.config.preserveCacheFiles[i])) {
        return true;
      }
    }
  }
    ...
    tree.local.list(Path.join(dirName, '/*'), function (err, items) {
      if (err) {
        ...
        async.each(items, function (item, eachCb) {
          if (item.isDirectory()) {
            if (item.getName() != constants.WORK_DIR) {
...
function _deleteDirs(tree, processDirs, skipDelete, cb) {
  async.each(processDirs, function (dir, eachCb) {
    ...
    if (skipDelete[dirName]) {
      ...
      if (dirName != '/') {
        ...
        tree.work.deleteDirectory(dirName, function (err) {
          if (err) {
            eachCb(err);
          } else {
...
function _deleteFile(tree, toDelete, cb) {
  ...
  if (!_preserveCacheFile.call(tree, toDelete.getPath())) {
    logger.debug('deleting file %s', toDelete.getPath());
    tree.canDelete(toDelete.getPath(), function (err, canDelete) {
      if (err) {
        cb(err);
      } else if (canDelete) {
        logger.debug('deleting file %s', toDelete.getPath());
        tree.work.delete(toDelete.getPath(), function (err) {
          if (err) {
            cb (err);
          } else {
    ...
    logger.debug('not deleting preserved file %s', toDelete.getPath());
    cb(null, true);
  }
}
...
RQTree.prototype.isCreatedLocally = function (name, cb) {
  ...
  var self = this;
  self.work.isCreatedLocally(name, function (err, exists) {
    if (err) {
      self.handleErr(cb, err);
    } else {
      cb(null, exists);
