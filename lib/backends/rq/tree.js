...
var RQTree = function (share, content, tempFilesTree) {
  ...
  this.createdFiles = {};
  this.processor = new RQProcessor(share, this.rq);
          ...
          if (exists) {
            self.local.list(pattern, function (err, localFiles) {
              if (err) {
                ...
                var parentRemoteUrl = self.getRemotePath(parentPath);
                self.rq.getRequests(parentRemoteUrl, function (err, requests) {
                  var i;
                  var lookup = {};
                  var rqFiles = [];
                  for (i = 0; i < remoteFiles.length; i++) {
                    var remoteUrl = self.getRemotePath(remoteFiles[i].getPath());
                    if (requests[remoteUrl] != 'DELETE' && requests[remoteUrl] != 'MOVE') {
                  ...
                  for (i = 0; i < localFiles.length; i++) {
                    ...
                    var remoteIndex = lookup[localFiles[i].getName()];
 ...
 */
RQTree.prototype.queueData = function (name, method, newName, isFolder) {
...
RQTree.prototype.createDirectory = function (name, cb) {
  ...
  self.local.createDirectory(name, function (err, file) {
    if (err) {
      ...
      self.queueData(name, 'PUT', null, true);
      cb(null, file);
    }
  });
};
