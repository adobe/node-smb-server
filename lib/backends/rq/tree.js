 ...
 * @param {String} path The path whose created file should be processed.
 ...
 */
RQTree.prototype.processCreatedFile = function (path, cb) {
  var self = this;
  self.local.open(path, function (err, file) {
    if (err) {
      cb(err);
    } else if (file.isFile()) {
      self.work.exists(path, function (err, exists) {
        if (err) {
          ...
          logger.debug('created file for %s exists, retrieving', path);
          self.work.open(path, function (err, createdFile) {
            if (err) {
              cb(err);
            } else {
              cb(null, createdFile);
          ...
          logger.debug('%s is a temp file, no created file will be made', path);
          cb(null, file);
        } else {
          logger.debug('created file for %s does not exist, creating now', path);
          self.work.createFile(path, function (err, createdFile) {
            if (err) {
              cb(err);
            } else {
              cb(null, createdFile);
    ...
    if (err) {
      ...
      self.local.open(path, function (err, localFile) {
        if (err) {
          ...
          self.processCreatedFile(path, function (err, createdFile) {
            if (err) {
              cb(err);
            } else if (createdFile) {
              ...
              self.work.exists(self.getCreateFileName(path), function (err, exists) {
                if (err) {
                  cb(err);
                } else {
                  logger.debug('%s: canDelete result based on creation file exists value of %s and modified diff of %d (%d - %d). threshold is %d', path, exists, modifiedDiff, lastModified, created, self.share.config.modifiedThreshold);
                  // a file can only be safely deleted if it hasn't been modified locally and wasn't created locally
                  cb(null, (modifiedDiff <= self.share.config.modifiedThreshold) && !exists);
                }
              });
            } else {
...
RQTree.prototype.deleteLocalDirectoryRecursive = function (name, cb) {
  ...
  var processDir = function (dirName, itemProcess, finished) {
    self.local.list(dirName, function (err, items) {
      if (err) {
        cb(err);
      } else {
          ...
          if (err) {
            ...
            self.local.delete(items[index].getPath(), function (err) {
              if (err) {
                ...
                RQFile.deleteWorkFiles(items[index].getPath(), self, function (err) {
                  if (err) {
                    cb(err);
                  } else {
