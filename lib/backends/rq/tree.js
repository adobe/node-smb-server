 ...
 * @param {File} newRemote If truthy, the existing remote File instance of the target path.
 * @param {function} cb Will be invoked when the operation is complete.
 * @param {string|Error} cb.err Will be truthy if there were errors.
 */
RQTree.prototype.renameLocal = function (oldName, newName, newRemote, cb) {
  this.local.renameExt(oldName, newName, newRemote, cb);
};
...
function _getReplaceInfo(oldName, newName, cb) {
  ...
  async.series([_isDirectory, _getOldInfo, _getNewInfo, _getRemoteInfo], function (err) {
    if (err) {
      cb(err);
    } else {
      cb(null, info);
  ...
  _getReplaceInfo.call(this, oldName, newName, function (err, info) {
    if (err) {
      ...
      if (info.oldExistsLocal) {
        self.renameLocal(oldName, newName, info.newRemote, function (err) {
          if (err) {
            self.handleErr(cb, err);
          } else {
