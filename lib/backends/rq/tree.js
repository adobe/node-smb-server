 ...
 */
var RQTree = function (share, content, tempFilesTree, options) {
  ...
  if (!options.remote) {
    options.remote = new DAMTree(share, content, tempFilesTree);
  }
  if (!options.local) {
    options.local = new FSTree(new FSShare('rqlocal', share.config.local));
  }
  if (!options.work) {
    options.work = new FSTree(new FSShare('rqwork', share.config.work));
  }
  ...
  Tree.call(this, share, content, tempFilesTree);
};
