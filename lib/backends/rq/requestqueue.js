 ...
 * @param {String|Exception} callback.err Will be truthy if the operation failed.
 * @param {Bool} callback.exists Will be true if the item exists in the queue, otherwise will be false.
 */
 ...
 * @param {Int} maxRetries The number of retries that a request should exceed before being purged.
 * @param {Function} callback Will be invoked when requests have been purged.
 * @param {String|Exception} callback.err Will be truthy if the purge enounctered errors.
...
RequestQueue.prototype.incrementRetryCount = function (path, name, delay, callback) {
    ...
    this.db.update({$and: [{path: path},{name: name}]}, {$inc: {retries: 1}, $set: {timestamp: newTime}}, function (err, numAffected) {
        if (err) {
            callback(err);
    ...
    self.db.find(self.getFindPathFilter(oldPath), function (err, docs) {
        if (err) {
            ...
            var updateDoc = function (index) {
                if (index < docs.length) {
                    self.db.update({_id: docs[index]._id}, {$set: { path: self.getNewPath(docs[index].path, oldPath, newPath), timestamp: newTimestamp } }, function (err, numAffected) {
                        if (err) {
                            callback(err);
                        } else {
                            updateDoc(index + 1);
