...
RequestQueue.prototype = {
    ...
    updatePath: function (oldPath, newPath, callback) {
        var self = this;
        self.db.update({path:oldPath}, {$set: {path: newPath}}, {multi: true}, function (err, numAffected) {
            if (err) {
                callback(err);
            } else {
                callback();
    ...
    removePath: function (path, callback) {
        this.db.remove({path: path}, {}, function (err, numAffected) {
            if (err) {
                callback(err);
            } else {
                callback();
    ...
    copyPath: function (oldPath, newPath, callback) {
        var self = this;
        self.db.find({path: oldPath}, function (err, docs) {
            if (err) {
                callback(err);
            } else {
