...
RequestQueue.prototype = {
     ...
     */
    exists: function (path, name, callback) {
        var self = this;
     ...
     */
    purgeFailedRequests: function(maxRetries, callback) {
        var self = this;
     ...
     */
    incrementRetryCount: function (path, name, callback) {
        this.db.update({$and: [{path: path},{name: name}]}, {$inc: {retries: 1}}, function (err, numAffected) {
            if (err) {
                callback(err);
