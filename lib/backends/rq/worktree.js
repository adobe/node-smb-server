...
RQWorkTree.prototype.createFile = function (path, cb) {
    ...
    if (!self.rqTree.isTempFileName(path)) {
        self.localTree.exists(path, function (err, exists) {
            if (err) {
                cb(err);
            } else if (!exists) {
                ...
                // already exists, just move along
                logger.warn('%s work file already exists, but attempting to create another', path);
                cb();
            }
        });
    } else {
                            ...
                            if (err) {
                                ...
                                self.localTree.exists(newCreateName, function (err, newCreateExists) {
                                    if (!newCreateExists) {
                                        ...
                                        // new create file already exists, nothing to be done
                                        cb();
                                    }
                                });
                            }
