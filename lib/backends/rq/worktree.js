...
function _getIsCreated(origPath, filePath, checkCreated, cb) {
  var self = this;
  logger.debug('_getIsCreated: entering with origPath=%s, filePath=%s, checkCreated=%s', origPath, filePath, checkCreated);
  if (checkCreated) {
    self.rqTree.isQueuedForDelete(origPath, function (err, queued) {
      if (err) {
        cb(err);
      } else if (queued) {
        logger.debug('%s is queued for delete, not creating a created file', origPath);
        cb();
      } else {
        logger.debug('%s is not queued for delete, isCreated=true', origPath);
        cb(null, true);
      }
    });
  } else {
    logger.debug('checkCreated false, isCreated=false');
    cb();
  }
}

function _writeWorkData(origPath, filePath, localModified, checkCreated, remoteModified, cb) {
  var self = this;
  logger.debug('_writeWorkData: entering with filePath=%s, localModified=%d, checkCreated=%s, remoteModified=%d', filePath, localModified, checkCreated, remoteModified);
  self.localTree.open(filePath, function (err, workFile) {
    if (err) {
      logger.error('unable to open work file for writing %s', filePath, err);
      cb(err);
    } else {
      _getIsCreated.call(self, origPath, filePath, checkCreated, function (createdErr, isCreated) {
        if (createdErr) {
          logger.error('unable to determine created status of work file %s', filePath, createdErr);
          cb(err);
        } else {
          var writeData = JSON.stringify({
            localModified: localModified,
            created: isCreated ? true : false,
...
function _createWorkFile(name, created, remoteModified, cb) {
  var self = this;
    ...
    self.localTree.exists(filePath, function (err, exists) {
      if (err) {
        ...
        self.localTree.createFile(filePath, function (err) {
          ...
          _writeWorkData.call(self, name, filePath, syncDate, created, remoteModified, cb);
        });
      }
    });
  } else {
...
RQWorkTree.prototype.createFile = function (path, cb) {
  var self = this;
  _createWorkFile.call(self, path, true, 0, cb);
};
...
RQWorkTree.prototype.createFileExisting = function (path, cb) {
  _createWorkFile.call(this, path, false, 0, cb);
};
...
RQWorkTree.prototype.rename = function (oldName, newName, cb) {
  ...
  self.delete(oldName, function (err) {
    if (err) {
      ...
      _createWorkFile.call(self, newName, true, 0, cb);
    }
  });
};
...
function _open(name, createIfNoExist, cb) {
  var self = this;
  var fileName = _getFileInfoPath(name);
      ...
      if (createIfNoExist) {
        self.localTree.open(name, function (err, localFile) {
          if (err) {
            ...
            self.createFileExisting(name, 0, function (err) {
              if (err) {
                cb(err);
              } else {
            ...
            // it's a directory
            RQWorkFile.createInstance(name, localFile, self, cb);
          }
        });
      } else {
        cb();
...
RQWorkTree.prototype.isCreatedLocally = function (name, cb) {
  var self = this;
  if (!self.rqTree.isTempFileName(name)) {
    _open.call(this, name, false, function (err, workFile) {
      if (err) {
        cb(err);
      } else if (!workFile) {
        cb(null, false);
      } else {
        cb(null, workFile.isCreated());
      }
    });
  } else {
    // temp files are never created locally
    cb(null, false);
  }
};
